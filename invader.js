  ...
  Game.prototype = {
    ...
    // **draw()** draws the game.
    draw: function(c, gameSize) {
      // Clear away the drawing from the previous tick.
      c.clearRect(0, 0, gameSize.x, gameSize.y);

      // Draw each body as a rectangle.
      for (var i = 0; i < this.bodies.length; i++) {
        drawRect(c, this.bodies[i]);
      }
    },
  ...
  // **drawRect()** draws passed body as a rectangle to `c`, the drawing context.
  var drawRect = function(c, body) {
    c.fillRect(body.center.x - body.size.x / 2, body.center.y - body.size.y / 2,
